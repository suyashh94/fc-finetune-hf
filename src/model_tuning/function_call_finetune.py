...
class ModelTrainer:
    ...
    def collate(self, elements):
        ...
        return {
            "input_ids": torch.tensor([e["input_ids"] for e in elements]).to(self.device),
            "labels": torch.tensor([e["labels"] for e in elements]).to(self.device),
            "attention_mask": torch.tensor([e["attention_mask"] for e in elements]).to(self.device),
        }
    ...
    def prepare_data(self):
        ...
        if self.args.incomplete_data_path:
            ...
            num_samples = int(len(complete_messages) * self.sampling_ratio)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--sampling_ratio", type=float, default=0.1)
